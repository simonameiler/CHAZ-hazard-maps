...
#     """
#     Interpolate hazard metric data from a GeoDataFrame onto a regular grid and save as NetCDF.
#     Only include points on land using a Natural Earth shapefile for landmasses.
#     """
#     Define the shapefile path
#     shapefile_path = os.path.join(SYSTEM_DIR, "ne_10m_land", "ne_10m_land.shp")
    
#     Load and buffer the land shapefile
#     land = gpd.read_file(shapefile_path)
#     land = land.buffer(buffer_dist)

#     Union all geometries into a single polygon and wrap in a GeoDataFrame
#     land_union = land.unary_union
#     land_gdf = gpd.GeoDataFrame(geometry=[land_union], crs=land.crs)

#     Spatial join to filter points on land
#     gdf_on_land = gdf.sjoin(land_gdf, how="inner", predicate="within")

#     If no points remain, print a message and exit
#     if gdf_on_land.empty:
#         print("No points remain on land after spatial join.")
#         return

#     lon = gdf_on_land.geometry.x.values
#     lat = gdf_on_land.geometry.y.values
...
#         if col in ["geometry", "index_right"]:  # Skip geometry and sjoin index
#             continue

#         values = gdf_on_land[col].values
#         grid_values = griddata((lon, lat), values, (lon_mesh, lat_mesh), method=method)
